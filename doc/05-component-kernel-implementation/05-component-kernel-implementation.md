...
I'm selecting the representation to use two maps and a queue. One map stores the number of shares for each ticker, while the other stores the price for each ticker. This should make the kernel methods easy to implement as I can update, add, and remove tickers directly using the maps key. Also, a queue of ticker symbols would be useful for removeAnyTicker(). The convention would be that both maps contain exactly the same ticker symbols, all share counts are nonnegative, all prices are nonnegative, and the queue contains each ticker exactly once.
